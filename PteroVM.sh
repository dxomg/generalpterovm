...
case $input in
    ...
    "https://github.com/termux/proot-distro/releases/download/v4.30.1/ubuntu-questing-${ARCH}-pd-v4.30.1.tar.xz"
    tar -xf /tmp/rootfs.tar.gz -C $ROOTFS_DIR --strip-components=1;;

    2)
    wget --tries=$max_retries --timeout=$timeout -O /tmp/rootfs.tar.gz \
    "https://dl-cdn.alpinelinux.org/alpine/v3.24/releases/x86_64/alpine-minirootfs-3.24.1-${ARCH}.tar.gz"
